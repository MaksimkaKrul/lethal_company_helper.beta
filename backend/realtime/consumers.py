...
class RoomConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        if self.user.is_authenticated:
            await RoomRepository.add_user_to_room(self.room_id, self.user.id)
        
        await self.broadcast_player_list()
        
        current_state = await RoomRepository.get_room_state(self.room_id)
    ...
    async def disconnect(self, close_code):
        if self.user.is_authenticated:
            await RoomRepository.remove_user_from_room(self.room_id, self.user.id)
    ...
    async def broadcast_player_list(self):
        players = await RoomRepository.get_players_list(self.room_id, self.user.id, self.user.is_authenticated)
