...
export default function Room({ user }) {
    ...
    const { status, players, quotas, museum, connect, disconnect, updateQuotas, updateMuseum, setEmoji } = useRoomStore();
    ...
    const handleFileChange = (event) => {
        ...
        reader.onload = (e) => {
            try {
                ...
                if (imported.quotas) updateQuotas(imported.quotas);
                if (imported.museum) updateMuseum(imported.museum);
        ...
        <div className="screen" style={{display:'block'}}>
            ...
            {status !== WebSocketStatus.OPEN && (
                <div style={{
                    ...
                    {status === WebSocketStatus.FAILED ? 'CRITICAL: TERMINAL OFFLINE' : 'RE-ESTABLISHING LINK...'}
                </div>
            )}
                ...
                <div className="tab-pane active" style={{background: '#111', border: '1px solid #333', borderTop: 'none'}}>
                    {activeTab === 'Quota' && (
                        <QuotaTable 
                            ...
                            strategy={QUOTA_STRATEGIES[DEFAULT_STRATEGY]}
                            onUpdate={updateQuotas} 
                            onAdd={() => updateQuotas([...quotas, createEmptyQuota(quotas.length+1)])} 
                            onDelete={() => updateQuotas(quotas.slice(0, -1))} 
                        />
                    )}
                    {activeTab === 'Museum' && <Museum collectedItems={museum} onUpdate={updateMuseum} />}
                    {activeTab === 'Players' && <PlayersList players={players} currentUser={user} onSetEmoji={setEmoji} />}
                </div>
            </div>
        </div>
    );
